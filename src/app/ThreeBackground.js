...
const ThreeBackground = () => {
  useEffect(() => {
    ...
    function addNewElement(newElement) {
      ...
      const newFloatArray = new Float32Array(
        diskGeometries.length + newElement.length
      );
    ...
    for (let i = 0; i < num; i++) {
      ...
      let index = 0;
      let originalPositionArray = diskGeometry.attributes.position.array; // 元の頂点座標配列
      let newPositionArray = new Float32Array(
        (originalPositionArray.length / 3 - 1) * 3
      ); // 内側を削除した新しい頂点座標配列
      for (let i = innerVertices * 3; i < originalPositionArray.length; i++) {
        newPositionArray[index++] = originalPositionArray[i];
      }
    ...
    geometry.setAttribute(
      "position",
      new THREE.BufferAttribute(diskGeometries, 3)
    );

    // マテリアルを設定
    const material = new THREE.PointsMaterial({
      size: 0.05, // ポイントのサイズ
      color: 0x800080, // ポイントの色
